		...
		switch (action) {
			case 'delete':
				await sock.sendMessage(jid, { 
					text: `\`\`\`@${sender.split('@')[0]} в этой группе запрещено отправлять ссылки!\`\`\``,
					mentions: [sender] 
				});
				break;
			...
			case 'kick':
				...
				await sock.sendMessage(jid, {
					text: `\`\`\`@${sender.split('@')[0]} был удален за отправку ссылки в чат!\`\`\``,
					mentions: [sender]
				});
				break;
			...
			case 'warn':
				...
				if (warningCount >= WARN_COUNT) {
					...
					await sock.sendMessage(jid, {
						text: `\`\`\`@${sender.split('@')[0]} был исключен потому что набрал ${WARN_COUNT} предупреждений за отправку ссылок в чат\`\`\``,
						mentions: [sender]
					});
				} else {
					await sock.sendMessage(jid, {
						text: `\`\`\`@${sender.split('@')[0]} получил предупреждение ${warningCount} из ${WARN_COUNT} за отправку ссылки\`\`\``,
						mentions: [sender]
					});
				}
				break;
