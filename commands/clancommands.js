...
async function create(sock, chatId, senderId, args, message) {
    ...
    if (/\s/.test(name)) {
        await sock.sendMessage(chatId, { text: '❌ Название клана должно быть одним словом без пробелов' }, { quoted: message });
        return;
    }
    if (name.length > 15) {
...
async function promote(sock, chatId, senderId, message) {
    ...
    if ((clan.veterans || []).includes(target)) {
        ...
        if ((clan.officers?.length || 0) >= lvlData.officers) {
            await sock.sendMessage(chatId, { text: `❌ Достигнут лимит офицеров (макс. ${lvlData.officers})` }, { quoted: message });
            return;
        }
